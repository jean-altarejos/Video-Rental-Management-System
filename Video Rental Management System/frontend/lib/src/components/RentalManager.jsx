...
// //     // Access the nested .data property from ApiResponse wrapper
// //     setCustomers(custRes.data.data || []);
// //     setMovies(movieRes.data.data || []);
// //     setActiveRentals(rentalRes.data.data || []);
// //   } catch (err) {
...
// // API Base URL - update port/protocol to match your C# backend
// const API_BASE_URL = 'https://localhost:7065/api';
...
//  * Handles: direct arrays [...], { data: [...] }, { items: [...] }, 
//  * and Paginated API wrappers like { data: { items: [...] } }
//  */
...
//   // Form State
//   const [selectedCustomerId, setSelectedCustomerId] = useState('');
//   const [dateRented, setDateRented] = useState(getLocalDateString());
//   const [selectedMovieIds, setSelectedMovieIds] = useState([]);
...
//       const extractedMovies = extractDataArray(movieRes);
//       const extractedCustomers = extractDataArray(custRes);
//       const extractedRentals = extractDataArray(rentalRes);

//       // Log to browser console for easy visual debugging
//       console.log('API Movies Response Raw:', movieRes.data);
//       console.log('Extracted Movies Array:', extractedMovies);

//       setMovies(extractedMovies);
//       setCustomers(extractedCustomers);
//       setActiveRentals(extractedRentals);

//       if (extractedMovies.length === 0) {
//         console.warn('Movies array extracted as empty. Check backend payload structure.');
//       }
//     } catch (err) {
//       console.error('Failed to load API data:', err);
//       setApiErrorMsg(
//         err.message || 'Failed to fetch data from API. Please verify server status.'
//       );
//     } finally {
//       setIsLoading(false);
//     }
...
//   // Safe Field Getters (Handles C# PascalCase and camelCase variations)
//   const getMovieId = (m) => m?.movieID ?? m?.movieId ?? m?.id ?? m?.movie_ID;
//   const getMovieTitle = (m) =>
//     m?.title ?? m?.Title ?? m?.movieName ?? m?.MovieName ?? m?.name ?? `Movie #${getMovieId(m)}`;
//   const getMovieAvailable = (m) =>
//     m?.numberAvailable ??
//     m?.NumberAvailable ??
//     m?.stockQuantity ??
//     m?.StockQuantity ??
...
//   const getCustomerId = (c) => c?.customer_ID ?? c?.customerID ?? c?.customerId ?? c?.id;
//   const getCustomerName = (c) =>
//     c?.customerName ??
//     c?.CustomerName ??
//     (c?.firstName && c?.lastName
//       ? `${c.firstName} ${c.lastName}`
//       : `Customer #${getCustomerId(c)}`);
...
//     const movieId = parseInt(movieIdStr, 10);
//     const movie = movies.find((m) => getMovieId(m) === movieId);

//     if (!movie) return;

//     const stock = getMovieAvailable(movie);
//     if (stock <= 0) {
//       alert(`"${getMovieTitle(movie)}" is currently out of stock!`);
//       return;
//     }

//     if (selectedMovieIds.includes(movieId)) {
//       alert('This movie is already in your current rental cart.');
//       return;
//     }
...
//     if (!dateRented) newErrors.dateRented = 'Date Rented is required.';
//     if (selectedMovieIds.length === 0) newErrors.movies = 'Select at least one movie.';
...
//       createdDate: nowIso,
//       modifiedDate: nowIso,
//       rentalDetails: selectedMovieIds.map((id) => ({
//         movie: id,
//         dateReturned: null,
//       })),
//     };
...
//       setSelectedCustomerId('');
//       setSelectedMovieIds([]);
//       setDateRented(getLocalDateString());
//       setErrors({});
//       loadInitialData();
//     } catch (err) {
//       console.error('Rental Creation Error:', err);
//       alert('Failed to save rental transaction. Check console logs.');
//     } finally {
//       setIsSubmitting(false);
//     }
//   };
...
//   // Return Movie
//   const handleReturnMovie = async (rentalDetailId) => {
...
//       {apiErrorMsg && <div style={styles.errorBanner}>{apiErrorMsg}</div>}
...
//               <div style={styles.fieldGroup}>
//                 <label style={styles.label}>Add Movie to Cart:</label>
//                 <select
//                   onChange={(e) => {
//                     handleAddMovieToCart(e.target.value);
//                     e.target.value = '';
...
//                     const available = getMovieAvailable(m);

//                     return (
//                       <option key={id} value={id} disabled={available <= 0}>
...
//                   <ul style={styles.movieList}>
//                     {selectedMovieIds.map((id) => {
//                       const item = movies.find((m) => getMovieId(m) === id);
//                       return (
//                         <li key={id} style={styles.movieListItem}>
...
//               <div style={styles.rentalList}>
//                 {activeRentals.map((rental) => {
//                   const rentalId = rental.rentalID ?? rental.rental_ID ?? rental.id;
//                   const details = rental.rentalDetails ?? rental.RentalDetails ?? [];
//                   const custId = rental.customer_ID ?? rental.customerID ?? rental.customerId;

//                   return (
//                     <div key={rentalId} style={styles.rentalCard}>
//                       <div style={styles.rentalHeader}>
//                         <strong>Rental #{rentalId}</strong> - Date:{' '}
//                         {rental.dateRented?.split('T')[0]}
//                       </div>
//                       <div style={{ fontSize: '13px', marginBottom: '8px' }}>
//                         Customer ID: {custId}
//                       </div>
...
//                             const detailId =
//                               detail.rentalDetailId ??
//                               detail.rentalDetail_ID ??
//                               detail.id ??
//                               idx;
//                             const isReturned = Boolean(detail.dateReturned);
//                             const movieId = detail.movieID ?? detail.movieId;

//                             // Lookup title if included or available in loaded catalog
//                             const matchedMovie = movies.find((m) => getMovieId(m) === movieId);
//                             const displayMovieName = detail.movie?.title ?? detail.movie?.movieName ?? getMovieTitle(matchedMovie);

//                             return (
//                               <tr key={detailId}>
//                                 <td>{displayMovieName}</td>
//                                 <td>
//                                   {isReturned ? (
//                                     <span style={{ color: 'green' }}>
//                                       Returned ({detail.dateReturned.split('T')[0]})
...
//                                     <button
//                                       type="button"
//                                       onClick={() => handleReturnMovie(detailId)}
//                                       style={styles.returnBtn}
//                                     >
//                                       Return Movie
//                                     </button>
...
export default function RentalManager() {
  ...
  const handleRentSubmit = async (e) => {
    ...
    try {
      ...
      // Optimistically update local movie stock in UI immediately
      setMovies((prevMovies) =>
        prevMovies.map((movie) => {
          const mId = getMovieId(movie);
          if (selectedMovieIds.includes(mId)) {
            const currentStock = getMovieAvailable(movie);
            return {
              ...movie,
              numberAvailable: Math.max(0, currentStock - 1),
              NumberAvailable: Math.max(0, currentStock - 1),
            };
          }
          return movie;
      ...
      // Refresh master list from backend to keep full sync
  ...
  const handleReturnMovie = async (rentalDetailId, movieId) => {
    const today = getLocalDateString();
    try {
      await axios.put(`${API_BASE_URL}/rentals/return`, {
        dateReturned: today,
      });

      // Optimistically increase available stock in local state UI
      if (movieId) {
        setMovies((prevMovies) =>
          prevMovies.map((movie) => {
            if (getMovieId(movie) === movieId) {
              const currentStock = getMovieAvailable(movie);
              return {
                ...movie,
                numberAvailable: currentStock + 1,
                NumberAvailable: currentStock + 1,
              };
            }
            return movie;
                        ...
                        <tbody>
                          {details.map((detail, idx) => {
                            ...
                            const displayMovieName =
                              detail.movie?.movieName ??
                              detail.movie?.MovieName ??
                              detail.movie?.title ??
                              detail.movie?.Title ??
                              (matchedMovie ? getMovieTitle(matchedMovie) : null) ??
                              `Movie #${movieId ?? 'N/A'}`;
                              ...
                              <tr key={detailId}>
                                ...
                                <td>
                                  {!isReturned ? (
                                    <button
                                      type="button"
                                      onClick={() => handleReturnMovie(detailId, movieId)}
                                      style={styles.returnBtn}
                                    >
                                      Return Movie
                                    </button>
